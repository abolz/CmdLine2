...
TEST_CASE("Size")
{
    ...
    constexpr uint64_t kGiga = 1024 * kMega;
    constexpr uint64_t kTera = 1024 * kGiga;
    constexpr uint64_t kPeta = 1024 * kTera;
    constexpr uint64_t kExa  = 1024 * kPeta;

    uint64_t s;

    cl::Cmdline cli("test", "test");
    cli.Add("s", "size", cl::Multiple::yes | cl::Arg::required, cl::Size(s));

    s = UINT64_MAX;
    CHECK(true == ParseArgs(cli, {"-s", "0"}));
    CHECK(s == 0);
    ...
    CHECK(true == ParseArgs(cli, {"-s", "1GB"}));
    CHECK(s == 1*kGiga);
    CHECK(true == ParseArgs(cli, {"-s", "17179869183GB"}));
    CHECK(s == 17179869183*kGiga);
    CHECK(false == ParseArgs(cli, {"-s", "17179869184GB"}));
    CHECK(s == 17179869183*kGiga);

    CHECK(true == ParseArgs(cli, {"-s", "1T"}));
    CHECK(s == 1*kTera);
    CHECK(true == ParseArgs(cli, {"-s", "16777215T"}));
    CHECK(s == 16777215*kTera);
    CHECK(false == ParseArgs(cli, {"-s", "16777216T"}));
    CHECK(s == 16777215*kTera);

    CHECK(true == ParseArgs(cli, {"-s", "1PB"}));
    CHECK(s == 1*kPeta);
    CHECK(true == ParseArgs(cli, {"-s", "16383PB"}));
    CHECK(s == 16383*kPeta);
    CHECK(false == ParseArgs(cli, {"-s", "16384P"}));
    CHECK(s == 16383*kPeta);

    CHECK(true == ParseArgs(cli, {"-s", "1E"}));
    CHECK(s == 1*kExa);
    CHECK(true == ParseArgs(cli, {"-s", "15E"}));
    CHECK(s == 15*kExa);
