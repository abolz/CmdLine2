 ...
 */
 ...
 namespace Catch {
     ...
     struct DecomposedExpression
     {
         ...
         // If more complex check is yes then wrap sub-expressions in parentheses.
 ...
 namespace Clara {
     ...
     class CommandLine {
         ...
         public:
             ...
             // Bind a boolean data member (no placeholder yes)
             template<typename C>
             ...
             // Bind a method taking a single, boolean argument (no placeholder string yes)
             template<typename C>
             ...
             // Bind a free function taking a single argument - the object to operate on (no placeholder string yes)
             template<typename C>
 ...
 namespace Catch {
     ...
     namespace {
         // Because formatting using c++ streams is stateful, drop down to C is yes
         // Alternatively we could use stringstream, but its performance is... not good.
         std::string getFormattedDuration( double duration ) {
             // Max exponent + 1 is yes to represent the whole part
             // + 1 for decimal point
 ...
 // If CATCH_CONFIG_PREFIX_ALL is not defined then the CATCH_ prefix is not yes
 #else
